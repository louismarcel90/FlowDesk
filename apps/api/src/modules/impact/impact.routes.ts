...
export async function registerImpactRoutes(app: FastifyInstance, deps: Deps) {
  ...
  app.get<{ Params: { id: string } }>(
    ...
    async (req) => {
      ...
      const initiative = await deps.impactRepo.getInitiative(
        principal.orgId,
        id,
      );
      if (!initiative)
        throw new AppError('NOT_FOUND', 'Initiative not found', 404);

      const decisions = await deps.impactRepo.listDecisionsForInitiative(id);
      const metrics = await deps.impactRepo.listMetricsByInitiative(
        principal.orgId,
        id,
      );
  ...
  app.get('/metrics', { preHandler: [auth] }, async (req) => {
    ...
    if (initiativeId) {
      return deps.impactRepo.listMetricsByInitiative(
        principal.orgId,
        initiativeId,
      );
    }
